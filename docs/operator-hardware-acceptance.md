...
| index/mask | int64 index, rank/non-index extent, explicit index value range and broadcast mask constraints | selected/gather/scatter/where output shape | index dim/value out of range, index rank/extent mismatch, scatter src mismatch, broadcast mismatch | `torch.gather.npu.yaml`, `torch.where.npu.yaml` | index-select, gather, scatter, select and where CPU/GPU shared-fingerprint compares passed; complex index shape/value predicates covered by contract tests; Torch/NPU evidence recorded |
