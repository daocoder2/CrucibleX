...
| index/mask | int64 index, rank/non-index extent, explicit index value range and broadcast mask constraints | selected/gather/scatter/where output shape | index dim/value out of range (automatic invalid cases carry shape-matching out-of-range values), index rank/extent mismatch, scatter src mismatch, broadcast mismatch | `torch.gather.npu.yaml`, `torch.where.npu.yaml` | index-select, gather, scatter, select and where CPU/GPU shared-fingerprint compares passed; complex index shape/value predicates covered by contract tests; Torch/NPU evidence recorded |
...
| conv/norm/attention | conv channel aliases, groups positive/divisibility, effective-kernel geometry, raw stride/padding/dilation; layer_norm suffix/affine/eps; QKV batch/head/embed/mask/dropout/causal predicates | formula-derived output shape/dtype plus validity predicates | explicit channel/groups/kernel/stride, normalized-shape/affine/eps, mask/dropout negatives; explicit negatives skip only shape relationships so facts cannot re-legalize them | `torch.conv2d.matrix.yaml`, `torch.layer-norm.matrix.yaml`, `torch.attention.matrix.yaml` | legal conv2d/layer_norm/attention CPU/GPU/NPU passed with device-tensor evidence and matching per-case fingerprints; matrix fixtures cover named negatives, not extra runtime evidence |
| ACLNN signature | native scalar/array ABI, output dtype/shape, lifecycle | parsed signature and output descriptors | tensor-list, optional tensor/scalar, dynamic output, non-ND format, stride and storage-offset preflight rejection | `aclnn.mean.npu.yaml` | ACLNN mean/max-dim/sort NPU evidence; unsupported ABI forms are parsed then rejected, pending native ownership/null/descriptor evidence |

复杂算子的 generated 示例会同时产生合法 case、`expected_invalid` case、解析后的 `resolved_operator_contract` 与输出 shape。当前 conv2d、layer_norm、scaled-dot-product-attention legal cases 已分别在 source-baked GPU/NPU image 中通过，三组 GPU/NPU fingerprints 分别为 `1f94ff90397328bb6ae8fabca657a0bb5964b221fb1141886384b79ff5e15a55`、`372a73310ea081ae9dc9117c69f8abc78a28ffbf1aa2b85c9f5a951cbbe778dd`、`1283b6d0315b5e432737225ea6c169a9ea19d690c0121efc9fc7568a3fcf4632`，且均为 `backend_dtype_source=device_tensor`。当前真实执行矩阵：CPU NumPy broadcast 已通过；CPU/GPU Torch mean、matmul、bmm、where、topk、index-select、gather、scatter、select、reshape、transpose、masked_fill、bf16，以及 conv2d、layer_norm、scaled-dot-product-attention 的合法 case 已在同一容器、同一 case fingerprint 下通过；NPU Torch/ACLNN 多条 lane 已通过。CPU/GPU/NPU 三侧聚合仍由 `scripts/cpu_gpu_npu_accuracy_gate.sh` 汇总；ACLNN 作为 NPU-side executor 单独记录，不能替代 CPU/GPU evidence。
